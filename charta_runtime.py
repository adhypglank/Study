...
def to_mq5(module: ChartaModule) -> str:
    lines = ["// Generated by Charta runtime; review before attaching to a live account.", "#property strict", "void OnTick()", "{", "   // Charta IR instructions are intentionally explicit for review."]
    for index, item in enumerate(module.instructions, 1):
        if item.op == "spok":
            data = item.data
...
def main() -> None:
    ...
    export_parser.add_argument("format", choices=("charta", "python", "mq5"))
    export_parser.set_defaults(action=lambda args: print({"charta": to_charta, "python": to_python, "mq5": to_mq5}[args.format](unpack(args.package)), end=""))
